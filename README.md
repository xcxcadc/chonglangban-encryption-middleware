...
它兼容当前主题和 EZ 的请求协议：前端使用 16 位十六进制字符作为 AES 密钥和 IV，使用 AES-CBC/PKCS7 加密逻辑路径，再进行双层 Base64 编码后放入 URL。中间件解密后将普通请求转发到 V2Board 的 `/api/v1`，订阅请求和白名单支付回调单独处理。协议细节和可变路径见 [协议与配置](docs/protocol-and-config.zh-CN.md)。
